...
import fdd_configs as fdd_configs
...
class Backend:
    def __init__(self):
        ...
        epoch = 1400
        net_gen.load_state_dict(torch.load(f'./checkpoint/gen/net_{epoch}.pth', map_location='cpu'))
        net_gen.eval()

        ae_net = networks.SimpleAutoEncoder().to(self.device)
        epoch = 150
        ae_net.load_state_dict(torch.load(f'./checkpoint/ae/{epoch}.pth', map_location='cpu'))
        ae_net.eval()

        seg_net = networks.SimpleSegmentation(32, 64).to(self.device)
        epoch = 100
        seg_net.load_state_dict(torch.load(f'./checkpoint/seg/{epoch}.pth', map_location='cpu'))
        seg_net.eval()
    ...
    def process(self, image: PIL.Image.Image, th1=0.0012, th2=0.04):
