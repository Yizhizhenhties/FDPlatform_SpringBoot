...
export default {
  ...
  methods: {
    ...
    showValidate() {
      ...
      this.$http
        .post("/api/validateimg/getvalidateimgs/", {
          params: {},
        })
        .then((response) => {
          this.va_uuid = response.data.groupuuid;
          for (let i = 0; i < 9; i++) {
            this.imgs[i].src = 'data:image/png;base64,' + response.data.data[i].va_img;
          }
    ...
    PassValidate() {
      ...
      this.$http
        ...
        .then((response) => {
          this.va_id = response.data.groupid;
          for (let i = 0; i < 9; i++) {
            this.imgs[i].src = 'data:image/png;base64,' + response.data.data[i].va_img;
          }
          this.validate_loading = false;
    ...
    GoValidate() {
      ...
      this.$http
        .post("/api/validateimg/validate/", {
          groupuuid: this.va_uuid, answer: check_str
        })
        .then((response) => {
